...
type SessionContextValue = { user: User | null; token: string | null; loading: boolean; login: (email: string, password: string) => Promise<void>; register: (name: string, email: string, phone: string, password: string) => Promise<void>; logout: () => Promise<void>; };
...
export function SessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => { const result = await api<{ token: string; user: User }>('/auth/login', { method: 'POST', body: JSON.stringify({ email, password }) }); await save(TOKEN_KEY, result.token); await save(USER_KEY, JSON.stringify(result.user)); globalThis.__MINIFY_TOKEN__ = result.token; setToken(result.token); setUser(result.user); router.replace('/(tabs)'); }, []);
  const register = useCallback(async (name: string, email: string, phone: string, password: string) => { const result = await api<{ token: string; user: User }>('/auth/register', { method: 'POST', body: JSON.stringify({ name, email, phone, password }) }); await save(TOKEN_KEY, result.token); await save(USER_KEY, JSON.stringify(result.user)); globalThis.__MINIFY_TOKEN__ = result.token; setToken(result.token); setUser(result.user); router.replace('/(tabs)'); }, []);
