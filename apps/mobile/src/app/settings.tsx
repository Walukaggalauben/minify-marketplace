...
export default function Settings(){
 ...
 return <View style={styles.screen}>
  <View style={styles.header}><Pressable onPress={()=>router.back()} style={styles.back}><Ionicons name="arrow-back" size={31} color={TEXT}/></Pressable><Text style={styles.title}>Settings</Text></View>
...
function SettingRow({row}:{row:Row}){return <Pressable onPress={row.action} style={styles.row}>
 <View style={[styles.icon,{backgroundColor:row.color}]}><Ionicons name={row.icon} size={27} color="#FFF"/></View>
 <Text style={styles.label} numberOfLines={1}>{row.label}</Text>{row.value&&<Text style={styles.value} numberOfLines={1}>{row.value}</Text>}<Ionicons name="chevron-forward" size={25} color={MUTED}/>
 </Pressable>}
const styles=StyleSheet.create({
 screen:{flex:1,backgroundColor:'#EAF3F8'},header:{height:88,backgroundColor:'#FFF',flexDirection:'row',alignItems:'center',paddingHorizontal:18,borderBottomWidth:1,borderBottomColor:'#D6E0E6'},back:{width:42},title:{fontSize:25,fontWeight:'700',color:TEXT,marginLeft:8},content:{paddingBottom:110},
 identity:{height:78,backgroundColor:'#FFF',flexDirection:'row',alignItems:'center',paddingHorizontal:16,borderBottomWidth:1,borderBottomColor:'#D6E0E6'},identityLogo:{width:78,height:58,resizeMode:'contain',marginRight:10},identityName:{fontSize:17,fontWeight:'800',color:TEXT},identityEmail:{fontSize:12,color:MUTED,marginTop:3},
 row:{minHeight:92,backgroundColor:'#FFF',borderBottomWidth:1,borderBottomColor:'#D6E0E6',flexDirection:'row',alignItems:'center',paddingHorizontal:16},icon:{width:62,height:62,borderRadius:15,alignItems:'center',justifyContent:'center'},label:{flex:1,fontSize:17,color:TEXT,marginLeft:16,fontWeight:'600'},value:{maxWidth:'35%',fontSize:15,color:MUTED,marginRight:9}
});
