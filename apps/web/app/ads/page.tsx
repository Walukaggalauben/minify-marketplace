...
export default function AdsPage(){
 ...
 const categoryFields:Field[]=selectedCategory&&!selectedCategory.children?.length?fallbackFields(selectedCategory.name,selectedCategory.parent?.name||''):[];\n const af=categoryFields;
 ...
 async function load(){setLoading(true);setError('');try{const p=new URLSearchParams({limit:String(limit),page:String(page),sort,order:sort==='price'?'asc':'desc'});if(q)p.set('q',q);if(category!=='All categories')p.set('category',category);
   Object.entries(attributeFilters).forEach(([k,v])=>{if(v)p.set('attr_'+k,v)});if(city!=='All Uganda')p.set('city',city);if(condition!=='Any condition')p.set('condition',condition.toUpperCase());if(min)p.set('minPrice',min);if(max)p.set('maxPrice',max);const d=await api('/ads?'+p.toString());setItems(Array.isArray(d)?d:(d.items||[]));setTotal(Number(d.total||d.count||0));}catch(e:any){setError(e.message||'Could not load adverts.');}finally{setLoading(false);}}
 function reset(){setQ('');setCategory('All categories');setCity('All Uganda');setCondition('Any condition');setMin('');setMax('');setAttributeFilters({});setPage(1);}
