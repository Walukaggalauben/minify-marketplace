...
export default function PaymentPanel({order,onPaid}:{order:any,onPaid?:()=>void}){
 ...
 return <div className="payment-panel"><div><span className="eyebrow">PAYMENT</span><h3>Mobile Money</h3><p className="muted">Amount: {money(order.total)}</p></div><button className="btn primary" onClick={pay} disabled={busy}>{busy?'Preparing…':'Continue to Mobile Money'}</button>{msg&&<div className="notice">{msg}</div>}<small className="muted">No payment is taken until a live provider is connected.</small></div>;
}
