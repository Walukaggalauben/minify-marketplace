...
export default function FavoritesPage(){
 ...
 useEffect(()=>{const raw=localStorage.getItem('minify_user');if(!raw){location.href='/login?next=/favorites';return}const u=JSON.parse(raw);api('/favorites/'+u.id).then((x:any[])=>setItems(Array.isArray(x)?x.map(f=>({...f,ad:f.Ad})).filter(f=>f.ad):[])).catch(()=>{}).finally(()=>setLoading(false))},[]);
