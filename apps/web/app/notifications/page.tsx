...
export default function Notifications(){
 ...
 return <><Header/><main className="container section"><div className="page-head"><div><span className="eyebrow">ACCOUNT</span><h1>Notifications</h1><p className="muted">Updates about your adverts, orders and marketplace activity.</p></div><button className="btn outline" onClick={all} disabled={marking||!items.some(n=>!n.readAt)}>{marking?'Marking…':'Mark all read'}</button></div>
  {busy?<div className="panel">Loading notifications…</div>:items.length===0?<div className="panel empty"><h3>You're all caught up</h3><p className="muted">New marketplace updates will appear here.</p><Link href="/" className="btn primary">Browse marketplace</Link></div>:<div className="notifications-list">{items.map(n=><button key={n.id} className={'notification '+(!n.readAt?'unread':'')} onClick={()=>read(n.id,n.link)}><div><b>{n.title}</b><p>{n.body}</p><small>{new Date(n.createdAt).toLocaleString()}</small></div>{!n.readAt&&<span className="notification-dot"/>}</button>)}</div>}
 </main></>;
}
